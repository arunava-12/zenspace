...
const Sidebar: React.FC<any> = ({
  ...
  return (
    ...
    >
        ...
        >
          ...
          {isOpen && (
            <>
              ...
              <ChevronDown
                size={14}
                className={`text-zinc-400 transition-transform ${showWsMenu ? "rotate-180" : ""}`}
              />
            </>
          )}
        </button>
            ...
            {workspaces.map((ws: Workspace) => (
              <div key={ws.id} className="flex items-center gap-2 group">
                <button
                  ...
                  className={`flex-1 flex items-center gap-3 p-2 rounded-lg text-sm transition-colors ${
                ...
                <button
                  onClick={() => {
                    if (activeWorkspace.id === ws.id) {
                      alert("Switch workspace before deleting.");
                      return;
                    }
                    deleteWorkspace(ws.id);
                  }}
                  className="opacity-0 group-hover:opacity-100 p-2 hover:bg-red-500/10 text-red-500 rounded-lg transition"
      ...
      {/* Sidebar Footer */}
      <div
        className={`p-5 border-t border-white/10 dark:border-white/5 flex flex-col gap-4 ${!isOpen && "items-center"}`}
      >
        {!isOpen && (
