...
const ProjectDetails: React.FC<ProjectDetailsProps> = ({ store }) => {
  ...
  const statusConfig = {
    Active: { icon: PlayCircle, color: 'text-blue-600', bg: 'bg-blue-500/10', border: 'border-blue-500/20' },
    Planning: { icon: Edit3, color: 'text-amber-600', bg: 'bg-amber-500/10', border: 'border-amber-500/20' },
    Completed: { icon: CheckCircle2, color: 'text-emerald-600', bg: 'bg-emerald-500/10', border: 'border-emerald-500/20' },
    'On Hold': { icon: PauseCircle, color: 'text-rose-600', bg: 'bg-rose-500/10', border: 'border-rose-500/20' },
    Cancelled: { icon: XCircle, color: 'text-zinc-600', bg: 'bg-zinc-500/10', border: 'border-zinc-500/20' },
  };
          ...
          responseSchema: {
            ...
            items: {
              ...
              properties: {
                ...
                type: { type: Type.STRING, description: 'Task, Bug, Feature, or Improvement' }
              },
  ...
  const availableUsersToAdd = users.filter((u: any) => !project.memberIds.includes(u.id));

  return (
    <div className="space-y-6 pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="space-y-1">
          <Link to="/projects" className="inline-flex items-center gap-1 text-sm text-zinc-500 hover:text-blue-500 transition-colors">
            <ArrowLeft size={14} /> Back to Projects
          </Link>
          <div className="flex items-center gap-4">
            <h1 className="text-3xl font-black tracking-tighter">{project.name}</h1>
            <div className={`flex items-center gap-1.5 px-3 py-1 rounded-full border text-[10px] font-black uppercase tracking-widest ${currentStatus.bg} ${currentStatus.color} ${currentStatus.border} shadow-sm`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        <div className="lg:col-span-1 space-y-6">
          <div className="glass-card p-6 rounded-[2rem] shadow-sm space-y-6 border-zinc-200 dark:border-white/5">
            <div>
              <h3 className="text-[10px] uppercase tracking-widest font-black text-zinc-500 mb-2">Description</h3>
              <p className="text-sm leading-relaxed text-zinc-600 dark:text-zinc-300 font-medium">{project.description}</p>
            </div>

            <div className="space-y-4 pt-2">
              {[
                { label: 'Project Lead', value: lead?.name, icon: UserIcon, color: 'blue' },
                { label: 'Timeline', value: `${project.startDate} — ${project.endDate}`, icon: Calendar, color: 'emerald' },
              ].map((item) => (
                <div key={item.label} className="flex items-center gap-3">
                  <div className={`p-2 glass-inset rounded-xl text-${item.color}-600 dark:text-${item.color}-400`}>
                    <item.icon size={16} />
                  </div>
                  <div>
                    <p className="text-[10px] uppercase font-black text-zinc-500">{item.label}</p>
            ...
            <div className="pt-4 border-t border-zinc-100 dark:border-white/5">
              <h3 className="text-[10px] uppercase tracking-widest font-black text-zinc-500 mb-4">Team Members</h3>
              <div className="flex flex-wrap gap-2">
                {project.memberIds.map((mid: string) => {
                  const m = users.find((u: any) => u.id === mid);
                  return (
                    <img key={mid} src={m?.avatar} className="w-8 h-8 rounded-full border-2 border-white dark:border-zinc-900 shadow-sm" title={m?.name} />
                  );
                })}
                <button 
          ...
          <div className="flex-1 flex flex-col min-h-[500px]">
                          ...
                          return (
                            ...
                            >
                              <td className="px-8 py-5">
                                <div className="flex items-center gap-4">
                                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-[10px] glass-inset ${
                                    task.type === 'Bug' ? 'text-rose-600 dark:text-rose-400' :
                                    task.type === 'Feature' ? 'text-blue-600 dark:text-blue-400' :
                                  ...
                                  <div className="flex flex-col">
                                    ...
                                    <span className="text-[10px] text-zinc-400 font-bold uppercase">{task.type}</span>
                                  </div>
                                </div>
                              </td>
                              <td className="px-8 py-5">
                                <div className="flex items-center gap-2">
                                  <img src={assignee?.avatar} className="w-7 h-7 rounded-full border border-white dark:border-zinc-800 shadow-sm" alt="" />
                                  <span className="text-xs font-bold">{assignee?.name}</span>
                                </div>
                              </td>
                              ...
                              <td className="px-8 py-5 text-xs text-zinc-500 font-bold tracking-widest">{task.dueDate}</td>
                              <td className="px-8 py-5 text-right">
                                <span className={`text-[10px] font-black px-3 py-1.5 rounded-full glass-inset shadow-sm uppercase tracking-widest`}>
                                  {task.status}
                                </span>
                              </td>
                            </tr>
              ...
              <div className="space-y-6 animate-in fade-in duration-500">
                <div className="flex items-center justify-between">
                  <div className="relative group">
                    <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-400 group-focus-within:text-blue-500" />
                    <input 
                      type="text" 
                ...
                <div className="grid grid-cols-1 gap-3">
                  {projectFiles.length === 0 ? (
                    <div className="py-32 text-center glass-card rounded-[2rem] border-dashed border-2 border-zinc-200 dark:border-white/10">
                      <FileText size={48} className="mx-auto text-zinc-200 dark:text-zinc-800 mb-4" />
                      <p className="text-zinc-500 font-bold">No documents uploaded yet.</p>
                      <p className="text-xs text-zinc-400 mt-1">Upload PDFs, DOCX or DOC files for this project.</p>
                    </div>
                  ) : (
                    projectFiles.map((file: FileAsset) => {
                      const uploader = users.find((u: any) => u.id === file.uploadedBy);
                      return (
                        <div key={file.id} className="glass-card p-4 rounded-2xl border border-zinc-200 dark:border-white/5 flex items-center justify-between group hover:shadow-lg transition-all hover:bg-white dark:hover:bg-white/[0.02]">
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              ...
                              <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest mt-0.5">
                                {file.size} • {file.type.toUpperCase()} • {file.createdAt}
                              </p>
                            </div>
                          </div>
                          <div className="flex items-center gap-6">
                            <div className="hidden md:flex items-center gap-2">
                               <img src={uploader?.avatar} className="w-6 h-6 rounded-full" />
                               <span className="text-xs font-bold text-zinc-500">{uploader?.name}</span>
                            </div>
                            <div className="flex items-center gap-2">
                               <a 
                                href={file.url} 
            ...
            {activeTab === 'discussion' && (
              <div className="flex-1 flex flex-col glass-card rounded-[2rem] border-zinc-200 dark:border-white/5 shadow-sm overflow-hidden animate-in fade-in duration-500 h-full">
                <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar max-h-[500px]">
                  {projectComments.length === 0 ? (
                    ...
                    projectComments.map((comment: any) => {
                      const sender = users.find((u: any) => u.id === comment.userId);
                      const isMe = sender?.id === currentUser.id;
                      return (
                        <div key={comment.id} className={`flex gap-3 ${isMe ? 'flex-row-reverse' : ''}`}>
                          <img src={sender?.avatar} className="w-8 h-8 rounded-full flex-shrink-0 mt-1 shadow-sm" alt="" />
                          <div className={`space-y-1 max-w-[80%] ${isMe ? 'text-right' : ''}`}>
                            <div className="flex items-center gap-2 px-1">
                               <span className="text-[10px] font-black uppercase tracking-widest text-zinc-500">{sender?.name}</span>
                               <span className="text-[10px] font-medium text-zinc-400 uppercase">{new Date(comment.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                            </div>
                            <div className={`p-4 rounded-2xl text-sm leading-relaxed shadow-sm ${isMe ? 'bg-blue-600 text-white rounded-tr-none' : 'bg-white dark:bg-white/5 border border-zinc-100 dark:border-white/5 rounded-tl-none font-medium'}`}>
                              {comment.content}
                            </div>
                          </div>
                        </div>
                ...
                <form onSubmit={handleSendMessage} className="p-4 bg-zinc-50 dark:bg-black/20 border-t border-zinc-100 dark:border-white/5">
                  <div className="relative group">
                    <input 
                      type="text" 
                      value={message}
                ...
                <div className="space-y-8 relative">
                  ...
                  {timelineEvents.map((event, i) => (
                    <div key={i} className="relative pl-12 group">
                      <div className={`absolute left-0 top-1 w-8 h-8 rounded-full border-4 border-zinc-50 dark:border-zinc-950 flex items-center justify-center z-10 bg-${event.color}-50 dark:bg-${event.color}-900/30 text-${event.color}-600 dark:text-${event.color}-400 shadow-sm`}>
                        <event.icon size={14} />
                      </div>
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
          ...
          <div className="glass-card w-full max-w-xl rounded-[2.5rem] border border-zinc-200 dark:border-white/10 shadow-2xl overflow-hidden animate-in zoom-in duration-500">
            <div className="p-10 space-y-8">
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    <span className={`text-[10px] font-black px-2 py-0.5 rounded-lg border uppercase tracking-widest ${
                      selectedTask.type === 'Bug' ? 'bg-rose-500/10 text-rose-600 border-rose-500/20' :
                      selectedTask.type === 'Feature' ? 'bg-blue-500/10 text-blue-600 border-blue-500/20' :
                      'bg-zinc-500/10 text-zinc-500 border-zinc-500/20'
                    }`}>
                      {selectedTask.type}
                    </span>
                    <span className="text-[10px] font-black text-zinc-400 uppercase tracking-widest">ID: {selectedTask.id}</span>
                  </div>
                  <h2 className="text-3xl font-black tracking-tighter leading-none">{selectedTask.title}</h2>
                </div>
                <button onClick={() => setSelectedTask(null)} className="p-2 hover:bg-zinc-100 dark:hover:bg-white/10 rounded-full transition-colors">
                  <X size={24} />
                </button>
              </div>
              ...
              <div className="grid grid-cols-2 gap-8 py-6 border-y border-zinc-100 dark:border-white/5">
                <div className="space-y-1">
                  <p className="text-[10px] font-black uppercase tracking-widest text-zinc-500">Assignee</p>
                  <div className="flex items-center gap-3">
                    <img 
                      src={users.find((u: any) => u.id === selectedTask.assigneeId)?.avatar} 
                      className="w-10 h-10 rounded-xl shadow-md" 
                    />
                    <div>
                      <p className="text-sm font-bold">{users.find((u: any) => u.id === selectedTask.assigneeId)?.name}</p>
                      <p className="text-[10px] text-zinc-400 font-bold uppercase">Member</p>
                    </div>
                  </div>
                </div>
                <div className="space-y-1">
                  ...
                  <div className="flex items-center gap-2">
                    <div className={`w-3 h-3 rounded-full ${
                      ...
                      selectedTask.status === 'In Progress' ? 'bg-blue-500 animate-pulse' :
                      'bg-zinc-300 dark:bg-zinc-600'
                    }`} />
                    <span className="text-sm font-bold">{selectedTask.status}</span>
                  </div>
                </div>
                <div className="space-y-1">
                  <p className="text-[10px] font-black uppercase tracking-widest text-zinc-500">Priority</p>
                  <div className="flex items-center gap-2">
                    <Tag size={14} className={
                      selectedTask.priority === 'High' ? 'text-rose-600' :
                      selectedTask.priority === 'Medium' ? 'text-amber-600' :
                      'text-zinc-500'
                    } />
                    <span className="text-sm font-bold">{selectedTask.priority}</span>
                  </div>
                </div>
                <div className="space-y-1">
                  <p className="text-[10px] font-black uppercase tracking-widest text-zinc-500">Due Date</p>
                  <div className="flex items-center gap-2">
                    <Calendar size={14} className="text-zinc-400" />
                    <span className="text-sm font-bold">{selectedTask.dueDate}</span>
                  </div>
                </div>
              </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-zinc-950/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="glass-card w-full max-w-md rounded-[2.5rem] border border-zinc-200 dark:border-white/10 shadow-2xl p-10 space-y-8 animate-in zoom-in duration-500">
            <div className="flex justify-between items-center">
               <h2 className="text-2xl font-black tracking-tighter">Add Member</h2>
               <button onClick={() => setShowAddMemberModal(false)} className="p-2 hover:bg-zinc-100 dark:hover:bg-white/10 rounded-full transition-colors">
                  <X size={20} />
               </button>
            </div>
            ...
            <div className="space-y-4 max-h-64 overflow-y-auto custom-scrollbar pr-2">
              {availableUsersToAdd.length === 0 ? (
                <p className="text-center text-zinc-500 text-sm py-4 font-bold">All workspace members are in the project.</p>
              ) : (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-left">
                        ...
                        <p className="text-[10px] text-zinc-400 font-black uppercase">{u.role}</p>
                      </div>
                    </div>
