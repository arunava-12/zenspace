        ...
        <div className="lg:col-span-2 space-y-10">
          <div className="glass-card rounded-[2rem] border border-zinc-200 dark:border-zinc-800 overflow-hidden shadow-sm">
            <div className="p-8 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between bg-white/30 dark:bg-white/5">
              <div className="flex items-center gap-3">
                 ...
                 <h2 className="font-bold text-xl tracking-tight">Focus Projects</h2>
              </div>
              <Link to="/projects" className="text-blue-500 hover:text-blue-600 text-sm font-bold inline-flex items-center gap-1 transition-colors">
                Explore All <ArrowRight size={14} />
              </Link>
            </div>
            <div className="divide-y divide-zinc-100 dark:divide-zinc-800">
              {projects.slice(0, 3).map((project: any) => (
                <div key={project.id} className="p-8 hover:bg-white/50 dark:hover:bg-zinc-800/30 transition-all group">
                  <div className="flex items-start justify-between">
                    <div className="space-y-1">
                      <Link to={`/projects/${project.id}`} className="text-lg font-bold text-zinc-900 dark:text-zinc-100 group-hover:text-blue-600 transition-colors tracking-tight">
                        {project.name}
                      </Link>
                      <p className="text-sm text-zinc-500 font-medium">{project.description}</p>
                    </div>
                    <span className={`text-[10px] px-3 py-1 rounded-full font-black uppercase tracking-widest ${
                      project.priority === 'High' ? 'bg-rose-500/10 text-rose-600' :
                      project.priority === 'Medium' ? 'bg-amber-500/10 text-amber-600' :
                  ...
                  <div className="mt-6 flex items-center gap-6">
                    ...
                    <span className="text-sm font-black text-zinc-900 dark:text-zinc-200">{project.progress}%</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="group relative bg-white dark:bg-zinc-950 rounded-[2.5rem] p-10 text-zinc-900 dark:text-white overflow-hidden shadow-2xl border border-zinc-200 dark:border-white/5 transition-colors duration-300">
             ...
             <div className="relative z-10 max-w-lg space-y-6">
                ...
                <h2 className="text-3xl font-black tracking-tighter">Meet ZenAI.</h2>
                <p className="text-zinc-500 dark:text-zinc-400 font-medium text-lg leading-relaxed">
                  Your team's intelligence layer. Generate roadmaps, brainstorm tasks, and summarize chaos into clarity.
                </p>
                <button 
                  onClick={openChatbot}
        ...
        <div className="space-y-10">
          <div className="glass-card rounded-[2rem] border border-zinc-200 dark:border-zinc-800 shadow-sm overflow-hidden flex flex-col">
            ...
            <div className="flex-1 p-6 space-y-4 max-h-[480px] overflow-y-auto custom-scrollbar">
              {myTasks.length === 0 ? (
                ...
                myTasks.map((task: any) => (
                  <div key={task.id} className="p-5 bg-white dark:bg-white/5 rounded-2xl border border-zinc-100 dark:border-white/5 hover:border-blue-500/20 transition-all cursor-pointer group shadow-sm hover:shadow-md">
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className={`text-sm font-bold tracking-tight truncate ${task.status === 'Done' ? 'line-through text-zinc-400' : ''}`}>
                          {task.title}
                        </p>
                        <div className="flex items-center gap-2 mt-2">
                           <Clock size={12} className="text-zinc-400" />
                           <p className="text-[10px] text-zinc-400 font-bold uppercase tracking-widest">{task.dueDate}</p>
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            <div className="p-6 border-t border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-white/[0.02]">
              <button onClick={() => navigate('/tasks')} className="w-full py-3 text-sm font-bold text-blue-600 hover:bg-blue-500/10 rounded-xl transition-all">
                Full View
              </button>
            </div>
          </div>
          ...
          <div className="glass-card rounded-[2rem] border border-zinc-200 dark:border-zinc-800 shadow-sm p-8 space-y-8">
                ...
                <div key={user.id} className="flex items-center justify-between group">
                  <div className="flex items-center gap-4">
                    <div className="relative">
                       <img src={user.avatar} className="w-12 h-12 rounded-2xl border-2 border-white dark:border-zinc-800 shadow-md group-hover:scale-110 transition-transform" alt="" />
            ...
            <button onClick={() => navigate('/team')} className="w-full py-4 bg-zinc-950 dark:bg-white text-white dark:text-zinc-950 rounded-2xl text-sm font-black transition-all hover:opacity-90 active:scale-95 shadow-xl">
              Directory
            </button>
          </div>
        </div>
