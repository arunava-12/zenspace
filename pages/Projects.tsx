...
import { Filter, Search, Plus, MoreVertical, Calendar, User as UserIcon } from 'lucide-react';
    ...
    <div className="space-y-8 pb-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-zinc-500 mt-1">Manage and track your team's project progress.</p>
        </div>
        <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {filteredProjects.map((project: any) => {
          ...
          const membersToShow = project.memberIds.slice(0, MAX_VISIBLE_MEMBERS);
          const remainingCount = project.memberIds.length - MAX_VISIBLE_MEMBERS;

          return (
            <div key={project.id} className="glass-card group flex flex-col rounded-2xl overflow-hidden hover:shadow-2xl hover:shadow-blue-500/5 transition-all duration-500 hover:-translate-y-1">
              <div className="p-6 flex-1 space-y-4">
                <div className="flex items-start justify-between">
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-xl font-bold shadow-inner ${
                    ...
                    project.status === 'Planning' ? 'bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20' :
                ...
                <div>
                  ...
                  <p className="text-sm text-zinc-500 mt-1 line-clamp-2 leading-relaxed">{project.description}</p>
                </div>

                <div className="flex flex-wrap gap-2 pt-1">
                  <span className={`text-[10px] uppercase tracking-wider font-bold px-2 py-0.5 rounded glass-inset ${
                    project.priority === 'High' ? 'text-rose-600 dark:text-rose-400' :
