...
import { User, Bell, Shield, Palette, Globe, CreditCard, LogOut, Moon, Sun, Monitor, CheckCircle2, Upload, Loader2 } from 'lucide-react';
...
const Settings: React.FC<SettingsProps> = ({ store }) => {
  const { currentUser, activeWorkspace, darkMode, setDarkMode, logout, updateProfile, updateAvatar, updateWorkspace } = store;
  ...
  const sections = [
    ...
    { name: 'Appearance', icon: Palette, desc: 'Customize theme and interface' },
    { name: 'Security', icon: Shield, desc: 'Password and authentication' },
    { name: 'Billing', icon: CreditCard, desc: 'Manage subscription and invoices' },
  ];
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
            <div className="flex items-center gap-6">
              <div className="relative group">
                <img src={currentUser.avatar} className="w-24 h-24 rounded-3xl border-4 border-white dark:border-zinc-800 shadow-xl object-cover transition-all" />
                <div 
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest ml-1">Job Title</label>
                <input 
                  type="text" 
                  value={jobTitle}
        ...
        return (
          <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
            <div className="space-y-4">
              <h3 className="text-[10px] font-black text-zinc-500 uppercase tracking-widest ml-1">Theme Mode</h3>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                >
                  <Sun className={!darkMode ? 'text-blue-500' : 'text-zinc-500'} size={24} />
                ...
                <button 
                  ...
                  className={`p-6 border rounded-[2rem] flex flex-col items-center gap-3 transition-all ${darkMode ? 'border-blue-500 bg-blue-500/5 ring-4 ring-blue-500/10' : 'border-zinc-200 dark:border-zinc-800 hover:bg-zinc-100 dark:hover:bg-zinc-800'}`}
                >
                  <Moon className={darkMode ? 'text-blue-500' : 'text-zinc-500'} size={24} />
            ...
            <div className="space-y-4">
              <h3 className="text-[10px] font-black text-zinc-500 uppercase tracking-widest ml-1">Accent Color</h3>
              <div className="flex gap-4">
                {['bg-blue-600', 'bg-purple-600', 'bg-emerald-600', 'bg-rose-600', 'bg-amber-600'].map(color => (
                  <button key={color} className={`w-10 h-10 rounded-full ${color} ring-offset-2 ring-offset-white dark:ring-offset-zinc-950 hover:ring-2 transition-all cursor-pointer`}></button>
                ))}
              </div>
            </div>
          </div>
        ...
        return (
          <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
            <div className="p-8 glass-inset rounded-[2rem] flex items-center justify-between border-zinc-200 dark:border-white/5">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-zinc-500 font-medium tracking-tight">Active Team Space • Created 4 months ago</p>
                </div>
              </div>
              <button className="text-xs font-black text-blue-500 hover:underline uppercase tracking-widest">Manage Team</button>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black text-zinc-500 uppercase tracking-widest ml-1">Workspace URL</label>
                <div className="flex">
                  <span className="bg-zinc-100 dark:bg-zinc-800 border border-r-0 border-zinc-200 dark:border-zinc-700 px-5 py-4 rounded-l-2xl text-sm text-zinc-500 font-medium">zenspace.app/</span>
                  <input type="text" defaultValue={wsName.toLowerCase().replace(' ', '-')} className="flex-1 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 py-4 px-5 rounded-r-2xl text-sm outline-none focus:ring-4 focus:ring-blue-500/10 font-bold" />
                </div>
              </div>
            </div>
          </div>
             ...
             <div className="space-y-6">
                <div className="flex items-center justify-between p-6 border border-zinc-200 dark:border-zinc-800 rounded-[2rem] bg-white/30 dark:bg-white/5">
                   <div>
                      ...
                      <p className="text-xs text-zinc-500 font-medium mt-1">Add an extra layer of security to your account.</p>
                   </div>
                   <button className="px-6 py-2.5 bg-zinc-950 dark:bg-white text-white dark:text-zinc-900 rounded-xl text-xs font-black uppercase tracking-widest active:scale-95 transition-all">Enable</button>
                </div>
                <div className="space-y-4">
                  <h3 className="text-[10px] font-black text-zinc-500 uppercase tracking-widest ml-1">Change Password</h3>
                  <div className="space-y-4">
                    <input type="password" placeholder="Current Password" className="w-full bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 py-4 px-5 rounded-2xl text-sm outline-none focus:ring-4 focus:ring-blue-500/10 font-medium" />
                    <input type="password" placeholder="New Password" className="w-full bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 py-4 px-5 rounded-2xl text-sm outline-none focus:ring-4 focus:ring-blue-500/10 font-medium" />
                  </div>
                </div>
             </div>
          </div>
             ...
             <div className="p-10 rounded-[2.5rem] bg-gradient-to-br from-zinc-900 to-zinc-950 text-white relative overflow-hidden shadow-2xl border border-white/5">
                <div className="relative z-10 space-y-8">
                   <div className="flex justify-between items-start">
                      <div>
                        <span className="text-[10px] font-black uppercase tracking-widest text-zinc-400">Current Plan</span>
                        <h3 className="text-4xl font-black mt-2 tracking-tighter">Pro Monthly</h3>
                      </div>
                      <span className="px-4 py-1.5 bg-blue-600 rounded-full text-[10px] font-black uppercase tracking-widest shadow-lg shadow-blue-500/20">Active</span>
                   </div>
                   <div className="h-px bg-white/10 w-full"></div>
                   <p className="text-zinc-400 text-sm font-bold tracking-tight">Your next billing cycle starts on June 12, 2024.</p>
                   <button className="bg-white text-zinc-950 px-8 py-4 rounded-2xl text-sm font-black transition-all hover:scale-105 active:scale-95 shadow-2xl">Upgrade to Enterprise</button>
                </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-10">
        ...
        <div className="md:col-span-3 glass-card rounded-[3rem] border border-zinc-200 dark:border-zinc-800 p-10 space-y-10 shadow-sm relative overflow-hidden flex flex-col min-h-[600px]">
          <div className="space-y-2 relative z-10">
            <h2 className="text-3xl font-black tracking-tight leading-none">{activeSection}</h2>
            <p className="text-sm text-zinc-500 font-bold uppercase tracking-widest opacity-80">{sections.find(s => s.name === activeSection)?.desc}</p>
          </div>
