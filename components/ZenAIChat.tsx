...
const ZenAIChat: React.FC<ZenAIChatProps> = ({
  isOpen,
  onClose,
  currentUser,
}) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        {
          role: "assistant",
          content:
            "I'm sorry, I encountered an error connecting to my neural core. Please try again in a moment.",
        },
      ]);
    } finally {
      setIsTyping(false);
      ...
      <div className="w-full max-w-xl h-full md:h-[90vh] glass-card rounded-none md:rounded-[2.5rem] border border-zinc-200 dark:border-white/10 shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-right-10 duration-500">
        ...
        >
          {messages.map((msg, idx) => (
            ...
            >
              ...
              <div
                className={`max-w-[85%] space-y-1 ${msg.role === "user" ? "text-right" : ""}`}
              >
                <div
                  className={`p-4 rounded-[2rem] text-sm leading-relaxed shadow-sm font-medium ${
                    msg.role === "user"
        ...
        <div className="p-6 bg-white/50 dark:bg-zinc-900/50 border-t border-zinc-200 dark:border-white/5">
          ...
          <div className="mt-4 flex items-center justify-center gap-4 text-[10px] font-black uppercase tracking-widest text-zinc-500">
            ...
            <button
              type="button"
              onClick={() => setInput("Brainstorm project goals")}
              className="hover:text-blue-500 transition-colors"
            >
              Brainstorm Goals
            </button>
            ...
            <button
              type="button"
              onClick={() => setInput("Help with Roadmap")}
              className="hover:text-blue-500 transition-colors"
            >
              Plan Roadmap
            </button>
